...
def get_args(args) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        "--parts-file",
        type=str,
        help="json file containing parts to pick from",
        required=True,
    )
    parser.add_argument(
        "-b", "--budget", type=int, help="budget in euros", required=True
    )
...
def main():
    ...
    try:
        ...
        add_additional_columns(original_parts)
    ...
    while not in_budget_flag and category_idx < 6:  # 6 categories
        ...
        for not_selected_parts_idx, row in not_selected_parts.iterrows():
            if prev_not_selected_idx:
                selected_parts = selected_parts.drop(
                    prev_not_selected_idx
                )  # We are trying again, so drop the last selected
            prev_not_selected_idx = not_selected_parts_idx
            print("#" * 40)
            print(f"Category Part Swap: {category}")
            print(f"\t- Removing: {deselected_part}")
            print(
                f"\t- Adding: {not_selected_parts.loc[not_selected_parts_idx]['name']}"
            )
